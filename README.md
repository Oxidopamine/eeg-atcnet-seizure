# EEG-ATCNet — Seizure Detection Extension (Bonn dataset)

> **This is a fork of [Altaheri/EEG-ATCNet](https://github.com/Altaheri/EEG-ATCNet)** extended to
> detect epileptic seizures on the **Bonn University EEG dataset**, comparing ATCNet against an
> EEGNet baseline under a leakage-safe evaluation. The original ATCNet README follows below.

## Seizure-detection pipeline

| File | Purpose |
|------|---------|
| [`preprocess_bonn.py`](preprocess_bonn.py) | Loads the 5 Bonn sets; **splits at the original-segment level before windowing** (prevents data leakage); supports `binary` / `3class` / `5class` schemes. |
| [`main_seizure.py`](main_seizure.py) | Trains ATCNet (single-channel) and EEGNet; reports accuracy, Cohen's κ, macro-AUC, and seizure sensitivity/specificity; saves figures + a results CSV. |
| [`report_utils.py`](report_utils.py) | Learning-curve, confusion-matrix, and ROC plotting helpers. |
| [`generate_report.py`](generate_report.py) | Builds a self-contained HTML report from the results (HTML/PDF not version-controlled). |
...
#   http://www.upf.edu/web/ntsa/downloads  (Andrzejak et al., 2001)

python main_seizure.py        # trains both models on binary + 3-class tasks
python generate_report.py     # builds report.html from the saved results
```

### Key results (held-out test set, leakage-safe split)

| Task | Model | Acc. | κ | AUC | Sz. Sens. | Sz. Spec. |
|------|-------|------|---|-----|-----------|-----------|
...
> **Note:** the compact EEGNet (~2k params) matches the much larger ATCNet (~114k params) on this
> single-channel data — ATCNet's spatial-attention advantage requires multi-channel input. Next
> step: the multi-channel TUH EEG corpus.

> _Portions of this extension (pipeline, evaluation code, and report) were developed with the
> assistance of the AI tool Claude._
